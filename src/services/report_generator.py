...
try:
    ...
    raise ImportError(
        f"No se puede generar PDF. Instale reportlab: pip install reportlab. Detalles: {str(e)}") from e
...
def generate_pdf_report(
    ...
    try:
        ...
        c.drawString(0.5 * inch, height - 0.5 * inch, "Reporte de Diagnóstico - Detector de Neumonía")

        # Línea separadora
        c.setLineWidth(1)
        c.line(0.5 * inch, height - 0.65 * inch,
               width - 0.5 * inch, height - 0.65 * inch)
        ...
        if heatmap_array is not None:
            try:
                ...
                temp_heatmap = os.path.join(
                    output_dir, f"temp_heatmap_{report_id}.png")
        ...
        c.drawString(0.5 * inch, 0.4 * inch, "Este reporte fue generado automáticamente por el Sistema de Detección de Neumonía")
